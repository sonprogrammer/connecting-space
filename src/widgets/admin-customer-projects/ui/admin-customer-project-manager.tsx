...
export function AdminCustomerProjectManager() {
  ...
  const [appliedProjectStatus, setAppliedProjectStatus] = useState<ProjectStatus | "">("");
  const [customerPage, setCustomerPage] = useState(1);
  const [projectPage, setProjectPage] = useState(1);
  const [selectedCustomerId, setSelectedCustomerId] = useState<string | null>(null);
  const [selectedProjectId, setSelectedProjectId] = useState<string | null>(null);
  const [customerNotice, setCustomerNotice] = useState<{ id: string; message: string } | null>(null);
  const [projectNotice, setProjectNotice] = useState<{ id: string; message: string } | null>(null);
  const queryClient = useQueryClient();
  const customerListQuery = useQuery({ queryKey: customerProjectQueryKeys.customers.list(customerPage, appliedCustomerQuery), queryFn: () => fetchCustomerList(customerPage, appliedCustomerQuery), placeholderData: keepPreviousData });
  const projectListQuery = useQuery({ queryKey: customerProjectQueryKeys.projects.list(projectPage, appliedProjectQuery, appliedProjectStatus), queryFn: () => fetchProjectList(projectPage, appliedProjectQuery, appliedProjectStatus), placeholderData: keepPreviousData });
  ...
  const customerDetail = toDetailState(customerDetailQuery, activeCustomerId, customerNotice?.id === activeCustomerId ? customerNotice.message : undefined, () => void customerDetailQuery.refetch());
  const projectDetail = toDetailState(projectDetailQuery, activeProjectId, projectNotice?.id === activeProjectId ? projectNotice.message : undefined, () => void projectDetailQuery.refetch());
  const linkedProject = toLinkedState(linkedProjectQuery, linkedProjectId, () => void linkedProjectQuery.refetch());
  const linkedCustomer = toLinkedState(linkedCustomerQuery, linkedCustomerId, () => void linkedCustomerQuery.refetch());
  useEffect(() => {
  ...
  const saveCustomer = (item: AdminCustomerDetail) => { queryClient.setQueryData(customerProjectQueryKeys.customers.detail(item.id), item); setCustomerNotice({ id: item.id, message: "고객 정보를 저장했습니다." }); void queryClient.invalidateQueries({ queryKey: customerProjectQueryKeys.customers.lists() }); };
  const saveProject = (item: AdminProjectDetail) => { queryClient.setQueryData(customerProjectQueryKeys.projects.detail(item.id), item); setProjectNotice({ id: item.id, message: "프로젝트 정보를 저장했습니다." }); void queryClient.invalidateQueries({ queryKey: customerProjectQueryKeys.projects.lists() }); };
  return (
    <section className="overflow-hidden rounded-lg border border-[#dfe3dc] bg-white">
      ...
      {tab === "customers" ? <CustomerPanel state={customerState} query={customerQuery} setQuery={setCustomerQuery} setPage={setCustomerPage} onSearch={() => { const next = applyAdminSearch(customerQuery); setAppliedCustomerQuery(next.query); setCustomerPage(next.page); }} onPage={setCustomerPage} onRetry={() => void customerListQuery.refetch()} selectedId={activeCustomerId} onSelect={selectCustomer} detail={customerDetail} onDetailRetry={() => void customerDetailQuery.refetch()} onSaved={(item) => { saveCustomer(item); }} linkedProject={linkedProject.status === "success" ? linkedProject.item : undefined} linkedProjectState={linkedProject} onLinkedProjectRetry={() => void linkedProjectQuery.refetch()} onProject={linkedProject.status === "success" ? () => selectProject(linkedProject.item) : undefined} /> : <ProjectPanel state={projectState} query={projectQuery} setQuery={setProjectQuery} status={projectStatus} setStatus={setProjectStatus} onSearch={() => { const next = applyAdminSearch(projectQuery, projectStatus); setAppliedProjectQuery(next.query); setAppliedProjectStatus(next.status ?? ""); setProjectPage(next.page); }} setPage={setProjectPage} onPage={setProjectPage} onRetry={() => void projectListQuery.refetch()} selectedId={activeProjectId} onSelect={selectProject} detail={projectDetail} onSaved={(item) => { saveProject(item); }} linkedCustomer={linkedCustomer.status === "success" ? linkedCustomer.item : undefined} linkedCustomerState={linkedCustomer} onCustomer={linkedCustomer.status === "success" ? () => selectCustomer(linkedCustomer.item) : undefined} />}
    </section>
  );
}
...
function queryErrorMessage(error: unknown, entity: string) {
  ...
  return getAdminQueryWarning(error, entity);
}
...
export function toListState<T extends { items: unknown[]; page: number; totalPages: number }>(query: { data?: T; isPending: boolean; isError: boolean; error?: unknown }, entity: string): AdminCustomerListState | AdminProjectListState {
  ...
  const warning = getCachedQueryWarning(Boolean(query.data), query.isError, query.error, entity);
  if (!query.data || query.data.items.length === 0) return { status: "empty", page: query.data?.page ?? 1, totalPages: query.data?.totalPages ?? 0, warning };
  return { status: "success", items: query.data.items as AdminCustomerListItem[] & AdminProjectListItem[], page: query.data.page, totalPages: query.data.totalPages, warning };
}

export function toDetailState<T>(query: { data?: T; isPending: boolean; isError: boolean; error?: unknown }, id: string | null, notice?: string, retry?: () => void): DetailState<T> {
...
function toLinkedState<T>(query: { data?: T; isPending: boolean; isError: boolean; error?: unknown }, id: string | null, retry?: () => void): LinkedState<T> {
  if (!id) return { status: "idle" };
  if (query.isPending && !query.data) return { status: "loading" };
...
function CustomerPanel(props: Readonly<{ state: AdminCustomerListState; query: string; setQuery: (value: string) => void; setPage: (value: number) => void; onSearch: () => void; onPage: (page: number) => void; onRetry: () => void; selectedId: string | null; onSelect: (item: AdminCustomerListItem) => void; detail: DetailState<AdminCustomerDetail>; onDetailRetry: () => void; onSaved: (item: AdminCustomerDetail) => void; linkedProject?: AdminProjectDetail; linkedProjectState: LinkedState<AdminProjectDetail>; onLinkedProjectRetry: () => void; onProject?: () => void }>) {
  const { state, query, setQuery, setPage, onSearch, onPage, onRetry, selectedId, onSelect, detail, onDetailRetry, onSaved, linkedProject, linkedProjectState, onLinkedProjectRetry, onProject } = props;
  return <><SearchBar value={query} onChange={setQuery} onSubmit={onSearch} /><div className="grid xl:grid-cols-[0.72fr_1.28fr]"><ListPane state={state} selectedId={selectedId} onSelect={(item) => { if ("email" in item) onSelect(item); }} setPage={setPage} onPage={onPage} onRetry={onRetry} kind="customer" /><CustomerEditor detail={detail} onDetailRetry={onDetailRetry} onSaved={onSaved} linkedProject={linkedProject} linkedProjectState={linkedProjectState} onLinkedProjectRetry={onLinkedProjectRetry} onProject={onProject} /></div></>;
}

function ProjectPanel(props: Readonly<{ state: AdminProjectListState; query: string; setQuery: (value: string) => void; status: ProjectStatus | ""; setStatus: (value: ProjectStatus | "") => void; setPage: (value: number) => void; onSearch: () => void; onPage: (page: number) => void; onRetry: () => void; selectedId: string | null; onSelect: (item: AdminProjectListItem) => void; detail: DetailState<AdminProjectDetail>; onSaved: (item: AdminProjectDetail) => void; linkedCustomer?: AdminCustomerDetail; linkedCustomerState: LinkedState<AdminCustomerDetail>; onCustomer?: () => void }>) {
  const { state, query, setQuery, status, setStatus, setPage, onSearch, onPage, onRetry, selectedId, onSelect, detail, onSaved, linkedCustomer, linkedCustomerState, onCustomer } = props;
  return <><SearchBar value={query} onChange={setQuery} onSubmit={onSearch}><select aria-label="프로젝트 상태" value={status} onChange={(event) => { setStatus(event.target.value as ProjectStatus | ""); }} className="h-9 rounded-md border border-[#d8d1c6] bg-white px-2 text-sm"><option value="">모든 상태</option>{projectStatuses.map((value) => <option key={value} value={value}>{projectStatusLabels[value]}</option>)}</select></SearchBar><div className="grid xl:grid-cols-[0.72fr_1.28fr]"><ListPane state={state} selectedId={selectedId} onSelect={(item) => { if ("status" in item) onSelect(item); }} setPage={setPage} onPage={onPage} onRetry={onRetry} kind="project" /><ProjectEditor detail={detail} onSaved={onSaved} linkedCustomer={linkedCustomer} linkedCustomerState={linkedCustomerState} onCustomer={onCustomer} /></div></>;
}

function ListPane({ state, selectedId, onSelect, setPage, onPage, onRetry, kind }: Readonly<{ state: AdminCustomerListState | AdminProjectListState; selectedId: string | null; onSelect: (item: AdminCustomerListItem | AdminProjectListItem) => void; setPage: (value: number) => void; onPage: (page: number) => void; onRetry: () => void; kind: "customer" | "project" }>) {
  ...
  if (state.status === "error") return <div className="flex min-h-64 flex-col items-center justify-center border-b border-[#e8ebe5] p-6 text-center xl:border-r xl:border-b-0"><AlertCircle aria-hidden className="size-8 text-[#b42318]" /><p className="mt-3 font-medium text-[#912018]">목록을 불러오지 못했습니다</p><p className="mt-2 text-sm text-[#617068]">{state.message}</p><Button type="button" variant="outline" size="sm" className="mt-4" onClick={onRetry}>다시 시도</Button></div>;
  if (state.status === "empty") return <div className="min-h-64 border-b border-[#e8ebe5] text-center text-sm text-[#617068] xl:border-r xl:border-b-0">{state.warning ? <QueryWarning message={state.warning} onRetry={onRetry} /> : null}<p className="p-8">조건에 맞는 {kind === "customer" ? "고객" : "프로젝트"}가 없습니다.</p></div>;
  return <div className="border-b border-[#e8ebe5] xl:border-r xl:border-b-0">{state.warning ? <QueryWarning message={state.warning} onRetry={onRetry} /> : null}<div className="max-h-[40rem] divide-y divide-[#edf0ea] overflow-y-auto">{state.items.map((item) => { const selected = item.id === selectedId; const project = kind === "project" ? item as AdminProjectListItem : undefined; const customer = kind === "customer" ? item as AdminCustomerListItem : undefined; return <button key={item.id} type="button" aria-current={selected ? "true" : undefined} onClick={() => onSelect(item)} className={`w-full px-5 py-4 text-left focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-[#2e6f4f] ${selected ? "bg-[#edf4ef]" : "hover:bg-[#f7f8f5]"}`}><span className="block font-semibold">{item.name}</span><span className="mt-1 block text-xs text-[#617068]">{customer?.company_name || customer?.email || (project ? projectStatusLabels[project.status] : "연결 정보 없음")}</span>{project?.contract_amount != null ? <span className="mt-1 block text-xs text-[#617068]">계약금 {project.contract_amount.toLocaleString()}원</span> : null}</button>; })}</div><Pagination page={state.page} totalPages={state.totalPages} setPage={setPage} onPage={onPage} /></div>;
}

function QueryWarning({ message, onRetry }: Readonly<{ message: string; onRetry: () => void }>) {
  return <div role="alert" className="flex items-center justify-between gap-3 border-b border-[#f0d5a8] bg-[#fff8e8] px-4 py-3 text-xs text-[#7a4b00]"><span>{message}</span><Button type="button" variant="outline" size="xs" onClick={onRetry}>다시 시도</Button></div>;
}
...
function Links({ inquiryId, customer, project, customerState, projectState, onCustomer, onProject, onLinkedProjectRetry, onLinkedCustomerRetry }: Readonly<{ inquiryId?: string | null; customer?: AdminCustomerDetail; project?: AdminProjectDetail; customerState?: LinkedState<AdminCustomerDetail>; projectState?: LinkedState<AdminProjectDetail>; onCustomer?: () => void; onProject?: () => void; onLinkedProjectRetry?: () => void; onLinkedCustomerRetry?: () => void }>) {
