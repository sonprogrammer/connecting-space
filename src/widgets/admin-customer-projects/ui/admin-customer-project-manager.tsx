...
function ProjectPanel(props: Readonly<{ state: AdminProjectListState; query: string; setQuery: (value: string) => void; status: ProjectStatus | ""; setStatus: (value: ProjectStatus | "") => void; setPage: (value: number) => void; onSearch: () => void; onPage: (page: number) => void; onRetry: () => void; selectedId: string | null; onSelect: (item: AdminProjectListItem) => void; detail: DetailState<AdminProjectDetail>; onDetailRetry: () => void; onSaved: (item: AdminProjectDetail) => void; linkedCustomer?: AdminCustomerDetail; linkedCustomerState: LinkedState<AdminCustomerDetail>; onLinkedCustomerRetry: () => void; onCustomer?: () => void }>) {
  const { state, query, setQuery, status, setStatus, setPage, onSearch, onPage, onRetry, selectedId, onSelect, detail, onDetailRetry, onSaved, linkedCustomer, linkedCustomerState, onLinkedCustomerRetry, onCustomer } = props;
  return <><SearchBar value={query} onChange={setQuery} onSubmit={onSearch}><select aria-label="프로젝트 상태" value={status} onChange={(event) => { setStatus(event.target.value as ProjectStatus | ""); }} className="h-9 rounded-md border border-[#d8d1c6] bg-white px-2 text-sm"><option value="">모든 상태</option>{projectStatuses.map((value) => <option key={value} value={value}>{projectStatusLabels[value]}</option>)}</select></SearchBar><div className="grid xl:grid-cols-[0.72fr_1.28fr]"><ListPane state={state} selectedId={selectedId} onSelect={(item) => { if ("status" in item) onSelect(item); }} setPage={setPage} onPage={onPage} onRetry={onRetry} kind="project" /><div><ProjectEditor detail={detail} onDetailRetry={onDetailRetry} onSaved={onSaved} linkedCustomer={linkedCustomer} linkedCustomerState={linkedCustomerState} onLinkedCustomerRetry={onLinkedCustomerRetry} onCustomer={onCustomer} />{selectedId ? <div className="px-5 pb-6 sm:px-6"><ProjectPaymentManager key={selectedId} projectId={selectedId} /></div> : null}</div></div></>;
}
