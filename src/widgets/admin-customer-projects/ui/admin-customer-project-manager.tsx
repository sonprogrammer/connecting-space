...
import {
  ...
  applyAdminSearch, getAdminQueryWarning, getInquiryAnchorHref, getSaveFailure, projectStatusLabels, projectStatuses, projectToFormValues,
  resolveSelectedId,
...
type DetailState<T> = { status: "idle" } | { status: "loading" } | { status: "success"; item: T; notice?: string; warning?: string } | { status: "error"; message: string };
type LinkedState<T> = { status: "idle" } | { status: "loading" } | { status: "success"; item: T; warning?: string } | { status: "error"; message: string };
...
export function AdminCustomerProjectManager() {
  ...
  const customerDetail = toDetailState(customerDetailQuery, activeCustomerId, customerNotice?.id === activeCustomerId ? customerNotice.message : undefined);
  const projectDetail = toDetailState(projectDetailQuery, activeProjectId, projectNotice?.id === activeProjectId ? projectNotice.message : undefined);
...
export function toListState<T extends { items: unknown[]; page: number; totalPages: number }>(query: { data?: T; isPending: boolean; isError: boolean; error?: unknown }, entity: string): AdminCustomerListState | AdminProjectListState {
  ...
  return { status: "success", items: query.data.items as AdminCustomerListItem[] & AdminProjectListItem[], page: query.data.page, totalPages: query.data.totalPages, warning: query.isError ? queryErrorMessage(query.error, entity) : undefined };
}

export function toDetailState<T>(query: { data?: T; isPending: boolean; isError: boolean; error?: unknown }, id: string | null, notice?: string): DetailState<T> {
  if (!id) return { status: "idle" };
  if (query.isPending && !query.data) return { status: "loading" };
  if (query.isError && !query.data) return { status: "error", message: queryErrorMessage(query.error, "상세") };
  return query.data ? { status: "success", item: query.data, notice, warning: query.isError ? queryErrorMessage(query.error, "상세") : undefined } : { status: "loading" };
}
...
function toLinkedState<T>(query: { data?: T; isPending: boolean; isError: boolean; error?: unknown }, id: string | null): LinkedState<T> {
  ...
  return query.data ? { status: "success", item: query.data, warning: query.isError ? queryErrorMessage(query.error, "연결 데이터") : undefined } : { status: "loading" };
}
...
function CustomerPanel(props: Readonly<{ state: AdminCustomerListState; query: string; setQuery: (value: string) => void; setPage: (value: number) => void; onSearch: () => void; onPage: (page: number) => void; onRetry: () => void; selectedId: string | null; onSelect: (item: AdminCustomerListItem) => void; detail: DetailState<AdminCustomerDetail>; onSaved: (item: AdminCustomerDetail) => void; linkedProject?: AdminProjectDetail; linkedProjectState: LinkedState<AdminProjectDetail>; onProject?: () => void }>) {
  const { state, query, setQuery, setPage, onSearch, onPage, onRetry, selectedId, onSelect, detail, onSaved, linkedProject, linkedProjectState, onProject } = props;
  return <><SearchBar value={query} onChange={setQuery} onSubmit={onSearch} /><div className="grid xl:grid-cols-[0.72fr_1.28fr]"><ListPane state={state} selectedId={selectedId} onSelect={(item) => { if ("email" in item) onSelect(item); }} setPage={setPage} onPage={onPage} onRetry={onRetry} kind="customer" /><CustomerEditor detail={detail} onSaved={onSaved} linkedProject={linkedProject} linkedProjectState={linkedProjectState} onProject={onProject} /></div></>;
}
...
function Links({ inquiryId, customer, project, customerState, projectState, onCustomer, onProject }: Readonly<{ inquiryId?: string | null; customer?: AdminCustomerDetail; project?: AdminProjectDetail; customerState?: LinkedState<AdminCustomerDetail>; projectState?: LinkedState<AdminProjectDetail>; onCustomer?: () => void; onProject?: () => void }>) {
  return <div className="flex flex-wrap items-center gap-2 text-xs text-[#617068]">
    ...
    {customerState?.status === "success" && customerState.warning ? <span role="alert" className="text-[#7a4b00]">{customerState.warning}</span> : null}
    {projectState?.status === "success" && projectState.warning ? <span role="alert" className="text-[#7a4b00]">{projectState.warning}</span> : null}
  </div>;
}
