...
import {
  ...
  getSaveFailure, projectStatusLabels, projectStatuses, projectToFormValues,
...
export function AdminCustomerProjectManager() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/admin/customers/${selectedCustomerId}`, { cache: "no-store", headers: { Accept: "application/json" } })
  ...
  useEffect(() => {
    ...
    void fetch(`/api/admin/projects/${selectedProjectId}`, { cache: "no-store", headers: { Accept: "application/json" } })
  ...
  const linkedProject = customerDetail.status === "success" && projectState.status === "success" ? projectState.items.find((item) => item.customer_id === customerDetail.item.id) : undefined;
  const linkedCustomer = projectDetail.status === "success" && customerState.status === "success" ? customerState.items.find((item) => item.id === projectDetail.item.customer_id) : undefined;
...
function CustomerPanel(props: Readonly<{ state: AdminCustomerListState; query: string; setQuery: (value: string) => void; page: number; setPage: (value: number) => void; onSearch: () => void; onPage: (page: number) => void; selectedId: string | null; onSelect: (item: AdminCustomerListItem) => void; detail: DetailState<AdminCustomerDetail>; onSaved: (item: AdminCustomerDetail) => void; linkedProject?: AdminProjectListItem; onProject?: () => void }>) {
  const { state, query, setQuery, page, setPage, onSearch, onPage, selectedId, onSelect, detail, onSaved, linkedProject, onProject } = props;
  return <><SearchBar value={query} onChange={setQuery} onSubmit={onSearch} /><div className="grid xl:grid-cols-[0.72fr_1.28fr]"><ListPane state={state} selectedId={selectedId} onSelect={(item) => { if ("email" in item) onSelect(item); }} page={page} setPage={setPage} onPage={onPage} kind="customer" /><CustomerEditor detail={detail} onSaved={onSaved} linkedProject={linkedProject} onProject={onProject} /></div></>;
}

function ProjectPanel(props: Readonly<{ state: AdminProjectListState; query: string; setQuery: (value: string) => void; status: ProjectStatus | ""; setStatus: (value: ProjectStatus | "") => void; page: number; setPage: (value: number) => void; onSearch: () => void; onPage: (page: number) => void; selectedId: string | null; onSelect: (item: AdminProjectListItem) => void; detail: DetailState<AdminProjectDetail>; onSaved: (item: AdminProjectDetail) => void; linkedCustomer?: AdminCustomerListItem; onCustomer?: () => void }>) {
  const { state, query, setQuery, status, setStatus, page, setPage, onSearch, onPage, selectedId, onSelect, detail, onSaved, linkedCustomer, onCustomer } = props;
  return <><SearchBar value={query} onChange={setQuery} onSubmit={onSearch}><select aria-label="프로젝트 상태" value={status} onChange={(event) => { setStatus(event.target.value as ProjectStatus | ""); }} className="h-9 rounded-md border border-[#d8d1c6] bg-white px-2 text-sm"><option value="">모든 상태</option>{projectStatuses.map((value) => <option key={value} value={value}>{projectStatusLabels[value]}</option>)}</select></SearchBar><div className="grid xl:grid-cols-[0.72fr_1.28fr]"><ListPane state={state} selectedId={selectedId} onSelect={(item) => { if ("status" in item) onSelect(item); }} page={page} setPage={setPage} onPage={onPage} kind="project" /><ProjectEditor detail={detail} onSaved={onSaved} linkedCustomer={linkedCustomer} onCustomer={onCustomer} /></div></>;
}
...
function Links({ inquiryId, customer, project, onCustomer, onProject }: Readonly<{ inquiryId?: string | null; customer?: AdminCustomerListItem; project?: AdminProjectListItem; onCustomer?: () => void; onProject?: () => void }>) { return <div className="flex flex-wrap gap-2 text-xs text-[#617068]">{inquiryId ? <span className="rounded bg-[#f5f6f3] px-2 py-1">문의 연결: {inquiryId}</span> : null}{customer ? <Button type="button" variant="outline" size="xs" onClick={onCustomer}>연결 고객: {customer.name}</Button> : null}{project ? <Button type="button" variant="outline" size="xs" onClick={onProject}>연결 프로젝트: {project.name}</Button> : null}</div>; }
