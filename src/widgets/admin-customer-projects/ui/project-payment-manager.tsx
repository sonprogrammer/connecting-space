...
function PaymentForm({
  ...
  return <form className="grid gap-3 rounded-lg border border-[#e5e9e2] bg-[#fbfcfa] p-4" onSubmit={async (event) => { event.preventDefault(); const message = amountErrorMessage(form.amount); if (message) { setError(message); return; } setError(undefined); await onSubmit(form); }}>
    <div className="grid gap-3 sm:grid-cols-4">
      ...
      <label className="grid gap-1 text-sm font-medium">금액<input aria-label="결제 금액" required inputMode="numeric" value={form.amount} disabled={saving} onChange={(event) => setForm({ ...form, amount: event.target.value })} className="h-9 rounded-md border border-[#d8d1c6] bg-white px-2 font-normal" placeholder="원" /></label>
...
function ReceiptForm({ paymentId, saving, onSubmit }: { paymentId: string; saving: boolean; onSubmit: (paymentId: string, form: ReceiptFormState) => Promise<void> }) {
  const [form, setForm] = useState(emptyReceiptForm);
  const [error, setError] = useState<string>();
  return <form className="mt-3 grid gap-2 rounded-md bg-white p-3" onSubmit={async (event) => { event.preventDefault(); const message = amountErrorMessage(form.amount); if (message) { setError(message); return; } setError(undefined); await onSubmit(paymentId, form); setForm(emptyReceiptForm()); }}>
    <div className="grid gap-2 sm:grid-cols-3"><label className="grid gap-1 text-xs font-medium">입금액<input aria-label="입금액" inputMode="numeric" value={form.amount} disabled={saving} onChange={(event) => setForm({ ...form, amount: event.target.value })} className="h-8 rounded-md border border-[#d8d1c6] px-2" /></label><label className="grid gap-1 text-xs font-medium">입금일<input aria-label="입금일" type="date" value={form.receivedAt} disabled={saving} onChange={(event) => setForm({ ...form, receivedAt: event.target.value })} className="h-8 rounded-md border border-[#d8d1c6] px-2" /></label><label className="grid gap-1 text-xs font-medium">메모<input aria-label="입금 메모" value={form.memo} disabled={saving} onChange={(event) => setForm({ ...form, memo: event.target.value })} className="h-8 rounded-md border border-[#d8d1c6] px-2" /></label></div>
    {error ? <p role="alert" className="text-xs text-[#912018]">{error}</p> : null}<Button type="submit" size="xs" disabled={saving}>입금 등록</Button>
  </form>;
}

function PaymentRowView({ payment, receipts, saving, onEdit, onDelete, onReceipt, onDeleteReceipt }: { payment: PaymentRow & { receivedAmount: number; outstandingAmount: number }; receipts: PaymentReceiptRow[]; saving: boolean; onEdit: () => void; onDelete: () => void; onReceipt: (paymentId: string, form: ReceiptFormState) => Promise<void>; onDeleteReceipt: (id: string) => void }) {
...
export function ProjectPaymentManager({ projectId }: Props) {
  ...
  const run = async (action: () => Promise<unknown>, message: string) => { setSaving(true); setError(undefined); setNotice(undefined); try { await action(); await refresh(message); } catch (cause) { setError(paymentErrorMessage(cause)); } finally { setSaving(false); } };
