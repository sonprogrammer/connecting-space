...
export const adminCreateInquirySchema = z.object({
  ...
  budgetMin: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).optional(),
  budgetMax: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).optional(),
...
export type AdminCreateInquiryInput = z.infer<typeof adminCreateInquirySchema>;
