...
> **에이전트 작업 필수 조건:** 이 계획을 작업 단위로 실행할 때 `superpowers:executing-plans`를 사용한다. 각 단계는 체크박스(`- [x]`)로 추적한다.
