...
describe("automation process route", () => {
  ...
  it("returns explicit Slack configuration details when a Slack job fails", async () => {
    process.env.AUTOMATION_PROCESS_SECRET = "route-test-secret";
    delete process.env.SLACK_INQUIRY_WEBHOOK_URL;
    delete process.env.ADMIN_BASE_URL;
    const route = await import("../src/app/api/internal/automation/process/route");

    const response = await route.POST(new Request("https://example.test/api/internal/automation/process", {
      method: "POST",
